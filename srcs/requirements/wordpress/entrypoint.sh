...
if ! wp core is-installed --allow-root; then
    ...
    echo "Vérification de l'existence de l'utilisateur 'jralph'..."
fi

if ! wp user get jralph --field=login --allow-root --path='/var/www/html'; then
        echo "Création de l'utilisateur 'jralph'..."
        wp user create jralph jralph@example.com --role=author --user_pass="$MYSQL_PASSWORD" --allow-root
    else
        echo "L'utilisateur 'jralph' existe déjà."
fi
cd /
